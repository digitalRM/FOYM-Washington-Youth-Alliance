    ...
    <div className="py-0 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="w-full h-fit grid sm:grid-cols-2 grid-cols-1 md:grid-cols-4 grid-rows-1 gap-2 max-w-7xl">
          {timeline.map((item) => (
            <div key={item.name} className="col-span-1 row-span-1 aspect-square w-full bg-white rounded-xl shadow-lg p-4 flex flex-col justify-end bg-cover bg-center" style={{ backgroundImage: `url(${item.image})` }}>
              <p className=" text-lg font-semibold leading-8 tracking-tight text-white ">{item.name}</p>
            </div>
          ))}
        </div>
      </div>
